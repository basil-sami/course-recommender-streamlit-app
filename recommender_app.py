...
def train(model_name, params):
    ...
    if model_name == backend.models[0]:
        ...
        st.success('Done!')
    if model_name == backend.models[1]:
        # Start training course similarity model
        with st.spinner('Training...'):
            time.sleep(0.5)
            backend.train(model_name,params)
...
# User profile model
    ...
    profile_sim_threshold = st.sidebar.slider('User Profile Similarity score',
...
# Clustering model
    ...
    cluster_no = st.sidebar.slider('Number of Clusters',
                                   ...
                                   value=20, step=1)
    params['cluster_no'] = cluster_no
    component_no = st.sidebar.slider('Number of components',
                                   min_value=0, max_value=50,
                                   value=20, step=1)
